...
from base.models import ScheduledCourse, Department, Course
...
department_miashs, period_1 , period_2, tutor_algo_prog, tutor_conception, tutor_blabla, \
course_type_algo, room_algo, course_blabla, course_algo, scheduled1, scheduled_blabla, \
...
def test_tutors_algo(client_query, tutor_algo_prog: Tutor, tutor_conception : Tutor, tutor_blabla : Tutor,
    ...
    """
        query {
            tutors (dept :\"MIASHS\", week : "{\\"week\\" : 7, \\"year\\" : 2021}") {
                edges {
                    node {
                        username
                    }
    ...
    assert tutor_blabla.username in data["username"]
    assert tutor_algo_prog.username in data["username"]
    assert tutor_conception.username in data["username"]
