...
const scheduledCourses: ScheduledCourses = {
    ...
    perDay: computed(() => {
        const out: { [day: string]: Array<ScheduledCourse> } = {}
        weekDays.list.value.forEach((wd : WeekDay) => {
            out[wd.date] = scheduledCourseStore.getScheduledCoursesForDay(wd)
        })
        return out
    }),
    perDayPerRoomFilterBySelectedDepartments: computed(() => {
        ...
        Object.entries(scheduledCourses.perDay.value).forEach((entry) => {
            ...
            out[day] = listGroupBy(courses, (scheduledCourse: ScheduledCourse) => `${scheduledCourse.room?.id}`)
        })
        return out
    }),
...
const roomReservations: RoomReservations = {
    list: ref([]),
    perDay: computed(() => {
        return listGroupBy(roomReservations.list.value, (reserv: RoomReservation) => {
...
const selectedDepartments = computed(() => {
    ...
    if (!selectedDepartment.value) {
        ...
        selected = departmentStore.getAllDepartmentsFetched
    } else {
        ...
        Object.entries(scheduledCourses.perDayPerRoom.value).forEach((entry) => {
            out[entry[0]] = Object.fromEntries(
                Object.entries(entry[1]).map((e) => {
                    ...
                    e[1].forEach((sCourse: ScheduledCourse) => {
                        // Get the course's department
                        let dept = sCourse.course.type.department
                        if (!dept) {
                        ...
                        let courseType = sCourse.course.type
                        if (!courseType) {
...
function updateReservationPeriodicities() {
    return api.fetch.reservationPeriodicities().then((value: ReservationPeriodicity[]) => {
...
function reservationRemoveAllSamePeriodicity(reservation: RoomReservation) {
    ...
    api.fetch.roomReservations({ periodicityId: periodicityId }).then((deletionList: RoomReservation[]) => {
        // Delete each reservation in the list
        deleteRoomReservations(deletionList)
            .then((_) => {
                // Remove the associated periodicity
                api.delete.reservationPeriodicity(periodicityId)
            })
...
function reservationRemoveFutureSamePeriodicity(reservation: RoomReservation) {
    ...
    // the selected
    return api.fetch.roomReservations({ periodicityId: periodicityId }).then((deletionList: any[]) => {
        // Filter the older reservations
        deletionList = deletionList.filter(
            (reserv: { date: string | number | Date }) => new Date(reserv.date).getTime() > reservationTime
        )
    ...
    return api.delete
        ...
        .then(
            (_: any) => {
                ...
                roomReservations.list.value = roomReservations.list.value.filter((r: RoomReservation) => r.id != reservation.id)
            },
            (reason: unknown) => parseReason(reason, handleReason)
        )
        .catch((reason: unknown) => parseReason(reason, handleReason))
}
...
onMounted(() => {
    ...
    if (departmentStore.getCurrentDepartment) {
        selectedDepartment.value = departmentStore.getCurrentDepartment

        courseTypes.perDepartment.value = {}
        const typesList: { [key: string]: Array<CourseType> } = {}
        let typesCounter = departmentStore.getAllDepartmentsFetched.length
        departmentStore.getAllDepartmentsFetched.forEach((dept: Department) => {
            api.fetch.courseTypes({ department: dept.abbrev }).then((value: CourseType[]) => {
